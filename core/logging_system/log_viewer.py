...
class LogViewer:
    ...
    def setup_ui(self):
        ...
        self.calc_text = tk.Text(calc_frame, height=25, wrap=tk.WORD, font=('TkDefaultFont', 11, 'bold'),
        ...
        # Create legend content with increased height
        legend_frame = ttk.Frame(footer_frame)
        legend_frame.pack(fill=tk.X, padx=10, pady=15)  # Increased padding
    ...
    def _show_basic_risk_calculation(self, session_data):
        ...
        if total_fields + total_data > 0:
            ...
            field_risk = min(60, total_fields * 5.0)
            self.calc_text.insert(tk.END, f"• Field Risk: min(60, {total_fields} × 5.0) = {field_risk}\n", "calculation")
